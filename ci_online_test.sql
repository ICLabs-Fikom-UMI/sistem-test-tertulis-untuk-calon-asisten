...
INSERT INTO `asisten` (`id_asisten`, `nim`, `nama_asisten`, `email`, `matkul_id`) VALUES
(1, '13020210023', 'Nisa', 'nisa@gmail.com', 1),
(3, '13020210020', 'Mala', 'mala@gmail.com', 5);


DELIMITER $$
CREATE TRIGGER `edit_user_asisten` BEFORE UPDATE ON `asisten` FOR EACH ROW UPDATE `asisten` SET `email` = NEW.email, `username` = NEW.nim WHERE `users`.`username` = OLD.nim
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `hapus_user_asisten` BEFORE DELETE ON `asisten` FOR EACH ROW DELETE FROM `users` WHERE `users`.`username` = OLD.nip
$$
DELIMITER ;
...
(2, 'Asisten', 'Pembuat Soal dan ujian'),
...
INSERT INTO `kelas_Asisten` (`id`, `kelas_id`, `dosen_id`) VALUES
(1, 3, 1),
(2, 2, 1),
(3, 1, 1),
(9, 2, 3),
...
ALTER TABLE `asisten`
  MODIFY `id_asisten` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
ALTER TABLE `asisten`
  ADD CONSTRAINT `asisten_ibfk_1` FOREIGN KEY (`matkul_id`) REFERENCES `matkul` (`id_matkul`);
...
ALTER TABLE `kelas_asisten`
  ADD CONSTRAINT `kelas_asisten_ibfk_1` FOREIGN KEY (`asisten_id`) REFERENCES `asisten` (`id_asisten`),
  ADD CONSTRAINT `kelas_asisten_ibfk_2` FOREIGN KEY (`kelas_id`) REFERENCES `kelas` (`id_kelas`);

ALTER TABLE `mahasiswa`
  ADD CONSTRAINT `mahasiswa_ibfk_2` FOREIGN KEY (`kelas_id`) REFERENCES `kelas` (`id_kelas`);

ALTER TABLE `m_ujian`
  ADD CONSTRAINT `m_ujian_ibfk_1` FOREIGN KEY (`asisten_id`) REFERENCES `asisten` (`id_asisten`),
...
ALTER TABLE `tb_soal`
  ...
  ADD CONSTRAINT `tb_soal_ibfk_2` FOREIGN KEY (`asisten_id`) REFERENCES `dosen` (`id_asisten`);
